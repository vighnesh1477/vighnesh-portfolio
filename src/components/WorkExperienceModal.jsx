...
const EXPERIENCE = [
  {
    ...
    overview:
      "Gained hands-on exposure to electrical distribution systems, safety operations, transformer units, wiring, cabling, and maintenance workflows. Observed how power is supplied across Udupi and surrounding regions. Strengthened practical engineering understanding.",
    highlights: [
      "Worked in Kenjoor Sub-Station (Udupi District)",
      "Understanding of transformers, feeders, and safety systems",
      "Observed practical electrical distribution workflows",
      "Gained real-world engineering exposure before B.E.",
    ],
    // Image URL used directly
    image: "https://placehold.co/520x192/0F172A/ffffff?text=MESCOM+Experience",
  },
  {
    id: "mcb",
    title: "MCB — Marali Chande Balaga",
    subtitle: "Traditional Drumming Team (Cultural & Stage Events)",
    year: "4 Years Experience",
    overview:
      "Performed as a Chande (instrument) artist for cultural festivals, temple events, and stage shows. Built strong rhythm skills, discipline, and teamwork. Improved stage presence and confidence.",
    highlights: [
      "Performed at multiple cultural and temple events",
      "4 years of Chande (traditional instrument) experience",
      "Enhanced teamwork, discipline & coordination",
      "Strong contribution to cultural community events",
    ],
    image: "https://placehold.co/520x192/1E3A8A/ffffff?text=MCB+Chande+Team",
  },
  {
    id: "future",
    title: "Internships — Coming Soon",
    subtitle: "Software Development & AI/ML Internships",
    year: "Future Timeline",
    overview:
      "This section will include upcoming internships in Software Development, AI/ML, Full-Stack Development, or Research roles. The modal is fully ready — you can update anytime.",
    highlights: [
      "Slot ready for future internships",
      "Add company name, timeline, responsibilities",
      "Add project details & achievements later",
      "No limitations — editable anytime",
    ],
    image: "https://placehold.co/520x192/4F46E5/ffffff?text=Future+Internships",
  },
];
...
// Calculates the normalized difference for coverflow positioning (circular array logic)
...
const ExperienceCard = ({ item, diff, setIndex, index }) => {
  ...
  // Define 3D transformation based on position (diff)
  if (isCenter) {
  ...
  return (
    <div
      ...
      style={{
        ...
        transition: "all 0.45s cubic-bezier(0.4, 0, 0.2, 1)", // Smooth CSS transition
      }}
      ...
      <div className="w-full h-48 bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center">
        <img
          ...
          onError={(e) => {
             ...
             e.target.src = "https://placehold.co/520x192/F59E0B/ffffff?text=Image+Missing";
           }}
        />
      </div>

      <div className="p-6 h-[calc(100%-12rem)] overflow-auto">
        <h4 className="text-lg font-bold text-gray-900 dark:text-gray-50">{item.title}</h4>
        <p className="mt-2 text-sm text-indigo-600 dark:text-indigo-400 font-medium">
          {item.year}
        </p>
        <p className="mt-3 text-sm text-gray-600 dark:text-gray-400">{item.subtitle}</p>
      </div>
    </div>
  );
};
...
export default function WorkExperienceModal({ onClose }) {
  ...
  // Navigation function for previous (-1) and next (1)
  const navigate = useCallback(
    (dir) => setIndex((prev) => (prev + dir + len) % len),
    [len]
  );

  // Keyboard controls for navigation and closing
  useEffect(() => {
    const onKey = (e) => {
      ...
      <div className="relative w-full max-w-7xl h-[95vh] sm:h-[88vh] bg-white dark:bg-gray-900 rounded-3xl shadow-2xl overflow-hidden flex flex-col transition-all duration-300">
        ...
        <div className="px-8 pt-8 pb-6 border-b border-gray-300 dark:border-gray-700 bg-white/70 dark:bg-gray-900/70 z-30">
          ...
          <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
            Use keyboard arrows (← and →) or click buttons to browse the timeline.
          </p>
        </div>

        {/* CONTENT AREA: Roller (Left) and Details (Right) */}
